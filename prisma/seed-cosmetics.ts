...
const TITLES = [
    ...
    // ── GP 상점 구매 칭호 ──
    // 팀 팬 칭호 (LCK 10팀 전체, "oo팀 팬" 형식으로 통일)
...
const FRAMES = [
    ...
    { name: 'T1 프레임', rarity: 'RARE', gpCost: 3000, description: 'T1 팀 컬러 프레임', imageUrl: '/frames/t1.png' },
    { name: 'GEN.G 프레임', rarity: 'RARE', gpCost: 3000, description: 'GEN.G 팀 컬러 프레임', imageUrl: '/frames/gen.png' },
...
const BACKGROUNDS = [
    ...
    { name: '페이커의 서재', rarity: 'RARE', gpCost: 3000, description: '전설의 선수 페이커를 테마로 한 배경', imageUrl: '/backgrounds/faker.jpg' },
    { name: 'T1 본진', rarity: 'RARE', gpCost: 2500, description: 'T1 팀 컬러 배경', imageUrl: '/backgrounds/t1.jpg' },
    { name: '월드컵 무대', rarity: 'EPIC', gpCost: 6000, description: '리그 오브 레전드 월드 챔피언십 배경', imageUrl: '/backgrounds/worlds.jpg' },
    ...
    { name: '갤럭시', rarity: 'EPIC', gpCost: 7000, description: '우주를 담은 신비로운 배경', imageUrl: '/backgrounds/galaxy.jpg' },
...
const STICKERS = [
    // 팀 로고 스티커
    { name: 'T1 로고', emoji: '🔴', rarity: 'UNCOMMON', gpCost: 200, relatedTeam: 'T1', description: 'T1 팀 로고 스티커' },
    { name: 'GEN.G 로고', emoji: '🔵', rarity: 'UNCOMMON', gpCost: 200, relatedTeam: 'GEN', description: 'Gen.G 팀 로고 스티커' },
    { name: 'HLE 로고', emoji: '🟠', rarity: 'UNCOMMON', gpCost: 200, relatedTeam: 'HLE', description: 'HLE 팀 로고 스티커' },
    { name: 'KT 로고', emoji: '🟡', rarity: 'UNCOMMON', gpCost: 200, relatedTeam: 'KT', description: 'KT Rolster 팀 로고 스티커' },
    { name: 'DK 로고', emoji: '🩵', rarity: 'UNCOMMON', gpCost: 200, relatedTeam: 'DK', description: 'Dplus KIA 팀 로고 스티커' },
    ...
    { name: '월즈 우승', emoji: '🌍', rarity: 'LEGENDARY', gpCost: 10000, description: '월드 챔피언십 우승을 기념하는 전설 스티커' },
    { name: '페이커', emoji: '👑', rarity: 'LEGENDARY', gpCost: 0, condition: 'OWN_FAKER_CHALLENGER_CARD', description: '페이커 챌린저 카드 보유 시 지급' },
...
const PLAYER_CARD_SKINS = [
    ...
    { name: '황금 카드', rarity: 'LEGENDARY', gpCost: 12000, description: '황금으로 도금된 카드', imageUrl: '/card-skins/gold.png' },
...
const AVATARS = [
    ...
    { name: 'T1 라이온', rarity: 'RARE', gpCost: 2000, description: 'T1 사자 마스코트 아바타', imageUrl: '/avatars/t1-lion.png' },
    { name: '페이커 실루엣', rarity: 'LEGENDARY', gpCost: 0, condition: 'OWN_FAKER_CARD', description: '페이커 카드 보유 시 해금', imageUrl: '/avatars/faker-silhouette.png' },
...
async function main() {
    ...
    // ── 구버전 팀 팬 칭호 정리 (이름 형식 통일 전 항목) ──
    const OLD_TEAM_TITLE_IDS = [
        'title-T1-열혈-팬',
        'title-GEN.G-지지자',
        'title-HLE-홍군',
        'title-KT-롤스터-팬',
        'title-DRX-드래곤',   // DRX는 현재 LCK 소속 아님
    ]
    const deleted = await prisma.cosmeticItem.deleteMany({
        where: { id: { in: OLD_TEAM_TITLE_IDS } }
    })
    if (deleted.count > 0) {
        console.log(`  🗑️  구버전 팀 팬 칭호 ${deleted.count}개 삭제`)
    }
    ...
    // 스티커 (emoji, relatedTeam, relatedPlayer는 CosmeticItem에 없는 필드이므로 제거)
    for (const s of STICKERS) {
        const { emoji, relatedTeam, relatedPlayer, ...cosmeticFields } = s as any
        // emoji를 imageUrl에 임시 저장 (텍스트 이모지)
